...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build class B/C PDF manifest for MinerU agent parsing.")
    p.add_argument(
        "--class-b-manifest",
        type=Path,
    ...
    p.add_argument(
        "--pdf-root",
        type=Path,
        default=Path("outputs/smj_merged_categorized/success/pdf"),
    )
    p.add_argument("--run-dir", type=Path, required=True)
    p.add_argument("--max-size-mb", type=int, default=10)
    return p.parse_args()
...
def main() -> None:
    ...
    manifest_path = run_dir / "manifest_pdf.jsonl"
    write_jsonl(manifest_path, manifest_rows)
    write_jsonl(run_dir / "manifest_no_pdf.jsonl", no_pdf_rows)
    write_jsonl(run_dir / "manifest_oversize.jsonl", oversize_rows)
    write_jsonl(run_dir / "manifest_unreadable.jsonl", unreadable_rows)
    ...
    write_json(run_dir / "manifest_pdf_summary.json", summary)
