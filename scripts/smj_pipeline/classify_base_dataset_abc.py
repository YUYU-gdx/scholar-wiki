...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Classify base dataset into Class A/B/C using existing qualifier.")
    p.add_argument(
        "--input-base-dataset",
        type=Path,
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/literature_base/class_abc"),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    qualifier_mod = _load_module(
        "smj_pipeline_extraction_qualifier_for_base_dataset",
        project_root / "scripts" / "smj_pipeline" / "extraction" / "qualifier.py",
    )
