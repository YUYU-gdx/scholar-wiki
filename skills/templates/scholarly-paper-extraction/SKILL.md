...
优先读取关联文献的 Markdown 正文文件（真实 `source_md_path` / `derived/mineru/latest/*.md`）。只有关联文献没有可用 Markdown 时，才退回读取真实 HTML/PDF 路径；不要为了跳转或阅读臆造 Markdown 路径。
然后访问对应关联文献，完成以下任务：
...
使用固定块语法写入当前新导入文献的 Markdown 文件中。Reader Note 只回写当前文献 md，不写入关联文献文件。
...
>
> Note:
> 关联文献：[关联文献标题](关联文献实际本地 Markdown 路径或 file URL)
>
>关联强度：strong | medium
>
...
引用本地文献，使用 Markdown 链接：
- 必须使用库中真实存在的文件路径，不要写占位路径或假定固定文件名。
- 关联文献链接优先写该文献的真实 Markdown 路径 `source_md_path`，例如 `.../derived/mineru/latest/<实际文件名>.md`；这样前端点击后进入 Markdown 阅读器。
- 如果关联文献没有可用 Markdown，只能写真实 HTML/PDF 路径；前端会按该路径直接打开对应文件类型的阅读器，不要臆造 Markdown 路径。
- Windows 路径统一 `/` 分隔；允许 `C:/.../<实际文件名>.md` 或 `file:///C:/.../<实际文件名>.md`，推荐 `file:///C:/.../<实际文件名>.md`。
