...
1. 对变量先使用 `graph_variable_neighbors(mode=exact)` 输入变量名查询是否已有同名变量。
2. 若未命中或命中项与论文中的变量定义不一致，再调 `graph_variable_neighbors(mode=semantic)` 输入变量的概念文本召回候选变量，并按概念一致性选择最合适的别名 `aliases` 扩展。
3. 使用 `graph_variable_neighbors(mode=semantic)` 输入每一个变量和关系的概念文本查询是否存在以下情况：
    - 判断是否同变量
    - 判断是否同关系
    - 判断是否同理论机制
    - 判断是否同研究情境
...
若有则使用tools进行深度调研并在新文献中与旧文献存在联系的段落下方进行笔记，要求记录：
    - 相关旧文献的绝对路径md链接
    - 同变量/同关系/同理论机制/同研究情境/存在扩展、挑战、冲突或边界条件及对应的具体描述
...
- 任何字符串的值，保留原文表述，禁止修改表述，禁止自行增加下划线
