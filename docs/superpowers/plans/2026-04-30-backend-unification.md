...
```
src/kn_graph/__init__.py                     â†?Package init, version
src/kn_graph/__main__.py                     â†?CLI entry point (serve, worker)
src/kn_graph/app.py                          â†?FastAPI app factory
src/kn_graph/config.py                       â†?Pydantic Settings
src/kn_graph/routers/__init__.py
src/kn_graph/routers/graph.py                â†?/graph/*, /paper/*, /variable/*
src/kn_graph/routers/chat.py                 â†?/chat/*
src/kn_graph/routers/literature.py           â†?/literature/*
src/kn_graph/routers/pipeline.py             â†?/v1/pipeline/*, /v1/jobs/*
src/kn_graph/routers/workspace.py            â†?/api/v2/workspace/*
src/kn_graph/models/__init__.py
src/kn_graph/models/graph.py                 â†?Pydantic models for graph domain
src/kn_graph/models/chat.py                  â†?Pydantic models for chat domain
src/kn_graph/models/literature.py            â†?Pydantic models for literature domain
src/kn_graph/models/pipeline.py              â†?Pydantic models for pipeline domain
src/kn_graph/models/workspace.py             â†?Pydantic models for workspace domain
src/kn_graph/services/__init__.py
src/kn_graph/services/graph_service.py       â†?Business logic extracted from serve_graph_api.py
src/kn_graph/services/chat_service.py       â†?Business logic for chat
src/kn_graph/services/literature_service.py  â†?Business logic for literature
src/kn_graph/services/pipeline_service.py    â†?Business logic for pipeline jobs
src/kn_graph/services/workspace_service.py   â†?Business logic for workspace layouts
src/kn_graph/workers/__init__.py
src/kn_graph/workers/celery_app.py           â†?Celery configuration + task definitions
```

### Modified files

```
pyproject.toml                              â†?Add kn_graph package, add pydantic + pydantic-settings deps
```

### Deleted files (Phase 4 only, after verification)

```
scripts/smj_pipeline/serve_graph_api.py     â†?Replaced by kn_graph package
scripts/smj_pipeline/serve_async_pipeline_api.py â†?Replaced by kn_graph package
```
...
- [ ] **Step 1: Create `src/kn_graph/models/__init__.py`** â€?re-exports all models.

- [ ] **Step 2: Create `src/kn_graph/models/graph.py`** â€?models for `/graph/*`, `/paper/*`, `/variable/*` endpoints. Must include: `GraphOverview`, `GraphFull`, `GraphNode`, `GraphEdge`, `ModerationLink`, `InteractionLink`, `IsolatedNode`, `GraphSearchParams`, `GraphSearchResponse`, `SearchResult`, `NeighborhoodParams`, `NeighborhoodResponse`, `PaperDetail`, `VariableDetail`.

- [ ] **Step 3: Create `src/kn_graph/models/chat.py`** â€?models for `/chat/*` endpoints. Must include: `CreateSessionRequest`, `ChatSession`, `SendMessageRequest`, `SendMessageResponse`, `SSEEvent`, `CodexConfig`, `CodexHealthResponse`, `PreflightCheck`, `PreflightResponse`, `ProviderConfig`.

- [ ] **Step 4: Create `src/kn_graph/models/literature.py`** â€?models for `/literature/*`. Must include: `LiteratureLibrary`, `LiteratureSearchParams`, `LiteratureAnswerRequest`, `LiteratureAnswerResponse`, `LiteratureImportRequest`.

- [ ] **Step 5: Create `src/kn_graph/models/pipeline.py`** â€?models for `/v1/*`. Must include: `PipelineJob`, `PipelineJobsResponse`, `PipelineJobResult`, `PipelineSubmitResponse`, `PipelineBatchSubmitResponse`, `JobStatus`, `JobStage`.

- [ ] **Step 6: Create `src/kn_graph/models/workspace.py`** â€?models for `/api/v2/workspace/*`. Must include: `WorkspaceLayout`, `WorkspaceLayoutList`.
...
## Task 3: Service Layer â€?Graph, Workspace, Literature
...
- [ ] **Step 2: Create `src/kn_graph/services/graph_service.py`** â€?extract graph data loading, search, neighborhood BFS, paper/variable detail from `serve_graph_api.py` lines handling `/graph/*`, `/paper/*`, `/variable/*`. The service class takes `Settings` and graph data path in constructor.

- [ ] **Step 3: Create `src/kn_graph/services/workspace_service.py`** â€?extract workspace layout CRUD from `serve_graph_api.py`. Wraps `WorkspaceLayoutStore`.

- [ ] **Step 4: Create `src/kn_graph/services/literature_service.py`** â€?extract literature search/answer/import from `serve_graph_api.py`. Wraps `LiteratureService`.
...
## Task 4: Service Layer â€?Chat + Pipeline

**Files:**
- Create: `src/kn_graph/services/chat_service.py`
- Create: `src/kn_graph/services/pipeline_service.py`

- [ ] **Step 1: Create `src/kn_graph/services/chat_service.py`** â€?extract chat session management, message dispatch, SSE streaming, Codex config, provider config from `serve_graph_api.py`. Wraps `ChatService`.

- [ ] **Step 2: Create `src/kn_graph/services/pipeline_service.py`** â€?extract job store, job lifecycle, submit, cancel, retry logic from `serve_async_pipeline_api.py`. Wraps `InMemoryJobStore`, `SqliteJobStore`, and Celery dispatcher.
...
## Task 5: Routers â€?Graph, Workspace, Literature, Health
...
- [ ] **Step 2: Create `src/kn_graph/routers/graph.py`** â€?FastAPI `APIRouter` with all `/graph/*`, `/paper/*`, `/variable/*` endpoints using Pydantic models and `GraphService`.

- [ ] **Step 3: Create `src/kn_graph/routers/workspace.py`** â€?`/api/v2/workspace/*` endpoints using `WorkspaceService`.

- [ ] **Step 4: Create `src/kn_graph/routers/literature.py`** â€?`/literature/*` endpoints using `LiteratureService`.

- [ ] **Step 5: Update `src/kn_graph/app.py`** â€?uncomment and activate the router includes for graph, workspace, literature. Add `/healthz` that also checks pipeline readiness.
...
Test: `curl http://127.0.0.1:8013/healthz` â†?`{"status":"ok"}`

Test: `curl http://127.0.0.1:8013/graph/overview` â†?JSON response
...
## Task 6: Routers â€?Chat, Pipeline
...
- [ ] **Step 1: Create `src/kn_graph/routers/chat.py`** â€?all `/chat/*` endpoints including SSE streaming via `sse-starlette`, Codex config, provider config. Uses `ChatService`.

- [ ] **Step 2: Create `src/kn_graph/routers/pipeline.py`** â€?all `/v1/*` endpoints including SSE for job events, file upload via `UploadFile`. Uses `PipelineService`.

- [ ] **Step 3: Update `src/kn_graph/app.py`** â€?add chat and pipeline router includes.
...
Test: `curl http://127.0.0.1:8013/chat/sessions?library_id=supply_chain` â†?JSON

Test: `curl http://127.0.0.1:8013/v1/pipeline/health` â†?`{"status":"ok","executor":"inline"}`
...
- [ ] **Step 3: Create `src/kn_graph/workers/celery_app.py`** â€?extract Celery configuration from `serve_async_pipeline_api.py`. Define task signatures for parse-extract pipeline.

- [ ] **Step 4: Update `src/kn_graph/app.py`** â€?add static files mount and include all routers.
...
- [ ] **Step 2: Update test configuration** â€?any tests that directly import from `serve_graph_api` or `serve_async_pipeline_api` must be updated to import from `kn_graph.app` or `kn_graph.routers.*`.
...
- [ ] **Step 4: Update `scripts/smj_pipeline/app_launcher.py`** â€?change it to launch `uv run python -m kn_graph serve --port 8013` instead of two separate servers.
