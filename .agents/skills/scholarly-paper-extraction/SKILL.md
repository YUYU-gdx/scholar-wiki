...
- graph_variable_neighbors：在已有知识图谱里查变量邻域关系，支持 exact（精确名）和 semantic（语义近邻）两种模式，用于同名变量消歧、判断是否已有相关变量与边关系。
- 你所拥有的通用工具如grep等
...
然后访问对应关联文献，完成以下任务：
1. 确认是否确实是相同变量，若是则将关联文献追加到新导入文献对应变量的aliases字段中
2. 挖掘概念相近 / 同变量 / 同关系 / 同机制 / 同情境 / 存在扩展 / 挑战旧文献 / 观点或结论冲突 / 发现边界条件的情况，并仅用于结构化抽取结果
...
## 输出边界（强约束）
- 禁止创建、追加、修改任何 Reader Note / 文献笔记 markdown 内容。
- 禁止编辑 `corpus/papers/**` 下的论文正文 markdown。
- 仅允许写入 `extract_result.json` 及提取流程要求的结果文件。
